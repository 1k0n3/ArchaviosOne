...
<body class="site app"><div class="shell"><aside class="side-nav">
  ...
  <label class="lang" title="Sprache"><select id="lang-sel"><option>Deutsch</option></select></label>
  <div class="foot">' . esc(cfg('base_url')) . '</div>
</aside><main class="content">' . $flashHtml . $body . '</main></div>
<script src="' . static_url('i18n.js') . '"></script><script src="' . static_url('site.js') . '"></script></body></html>';
}
...
function page_settings(array $user, array $devices, array $oauth, ?string $deviceCode, array $decks): string {
  ...
  $devHtml = $devices ? '<table><tbody>' . implode('', array_map(fn($d) => '<tr><td><b>' . esc($d['name']) . '</b><div class="muted small"><span>verbunden</span> <span>' . esc(substr($d['created_at'], 0, 10)) . '</span> · <span>zuletzt</span> <span>' . (esc(str_replace('T', ' ', substr((string)($d['last_sync_at'] ?: $d['last_seen_at'] ?: ''), 0, 16))) ?: 'nie') . '</span></div></td><td><form method="post" action="/settings/devices/' . esc($d['id']) . '/revoke">' . $csrf . '<button class="ghost small" type="submit">Trennen</button></form></td></tr>', $devices)) . '</tbody></table>' : '<p class="muted small">Noch kein Gerät verbunden.</p>';
  $sel = fn($d, $v) => $d['visibility'] === $v ? 'selected' : '';
  $deckHtml = $decks ? '<table><tbody>' . implode('', array_map(fn($d) => '<tr><td><b>' . esc($d['name']) . '</b><div class="muted small">' . (!empty($d['archived_at']) ? '<span class="arch">Archiv</span> · ' : '') . esc($d['format'] ?? '') . ($d['share_slug'] && $d['visibility'] !== 'private' ? ' · <a href="/d/' . esc($d['share_slug']) . '/">/d/' . esc($d['share_slug']) . '</a>' : '') . '</div></td><td><form method="post" action="/decks/' . esc($d['id']) . '/visibility" class="inline">' . $csrf . '<select name="visibility" class="vis vis-' . esc($d['visibility']) . '" title="Sichtbarkeit" onchange="this.form.submit()"><option value="private" ' . $sel($d, 'private') . '>Privat</option><option value="link" ' . $sel($d, 'link') . '>Link</option><option value="public" ' . $sel($d, 'public') . '>Öffentlich</option></select></form></td></tr>', $decks)) . '</tbody></table>' : '<p class="muted small">Noch keine Decks synchronisiert.</p>';
  ...
  <div class="settings-grid">
    ...
    <section class="panel"><h3>Companion verbinden</h3><p class="muted small">Im Tray-Menü des lokalen Tools „Mit Website verbinden…“ wählen und diesen Code eingeben. Er gilt 15 Minuten.</p>' .
      ...
      <h3 style="margin-top:18px">Verbundene Geräte</h3>' . $devHtml . (($v = app_version()) ? '<p class="muted small" style="margin-top:14px"><span>Website-Stand</span>: <span>' . esc($v['version']) . ($v['commit'] ? ' · ' . esc($v['commit']) : '') . ($v['build'] ? ' · ' . esc(substr(str_replace('T', ' ', $v['build']), 0, 16)) : '') . '</span></p>' : '') . '</section>
    ...
    <section class="panel wide"><h3 class="fold-head" data-fold="settings-decks">Deck-Freigaben <span class="cnt">' . count($decks) . '</span><span class="fold-chev"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg></span></h3><div class="fold-body"><p class="muted small">Privat: nur du. Link: jeder mit dem Link. Öffentlich: zusätzlich auf der Startseite und in deinem Profil.</p>' . ($decks ? '<input type="search" class="deck-q" placeholder="Decks durchsuchen …" autocomplete="off">' : '') . $deckHtml . '<p class="muted small deck-q-none" hidden>Keine Treffer</p></div></section>
  </div>';
}

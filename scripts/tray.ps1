...
function Start-Watcher {
  ...
  # --supervised: der Watcher darf sich bei neuem Programmstand beenden (Code 75), der Tray startet ihn dann neu
  $script:node = Start-Process -FilePath $nodeExe -ArgumentList "src\watch.js --supervised" -WorkingDirectory $script:dir -WindowStyle Hidden -PassThru
  Update-Status
}
...
# Die Aufgabenplanung antwortet langsam (CIM, 1–3 s); der Zustand wird deshalb zwischengespeichert und nur alle zwei Minuten
# oder nach einer Änderung neu gelesen – sonst hängt das Menü bei jedem Öffnen
$script:taskCache = $null; $script:taskCacheAt = [datetime]::MinValue
function Get-AutostartTask([bool]$fresh = $false) {
  if ($fresh -or ((Get-Date) - $script:taskCacheAt).TotalSeconds -gt 120) {
    $script:taskCache = Get-ScheduledTask -TaskName $script:taskName -ErrorAction SilentlyContinue
    $script:taskCacheAt = Get-Date
  }
...
$menu.Add_Opening({
  ...
  if ($script:miDeploy) { $script:miDeploy.Visible = (Test-Path (Join-Path $script:dir "deploy-config.json")) -and (Test-Path (Join-Path $script:dir "scripts\deploy.js")) }
...
# Eigene Website per FTP auf den lokalen Stand bringen (nur, wenn deploy-config.json und scripts/deploy.js lokal vorhanden sind)
...
# Hat sich der Watcher für ein Update beendet (Code 75), gleich wieder starten
function Test-WatcherUpdate {
  if ($null -ne $script:node -and $script:node.HasExited) {
    $code = $null; try { $code = $script:node.ExitCode } catch {}
    if ($code -eq 75) { $script:node = $null; Start-Watcher; Show-Balloon "Neue Programmversion geladen." }
  }
}
$script:timer.Add_Tick({ if ($script:taskCacheAt -eq [datetime]::MinValue) { [void](Get-AutostartTask $true) }; Test-WatcherUpdate; Update-Status })
$script:timer.Start()

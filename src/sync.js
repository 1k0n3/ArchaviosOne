...
function cfg() {
  try { return require("./paths").readConfig(); } catch (e) { return {}; }
}
...
/**
 * Zugang des Assistenten zur Website spiegeln, damit man dort ohne zweite Anmeldung verbunden ist.
 * Nur der letzte Stand zählt; der Schlüssel liegt auf dem Server verschlüsselt.
 */
function enqueueAssistant(cfg) {
  const body = { provider: cfg.provider || "", model: cfg.model || "", key: cfg.apiKey || "" };
  const q = path.join(syncDir(), "queue");
  if (fs.existsSync(q)) for (const f of fs.readdirSync(q)) if (f.startsWith("assistant_")) { try { fs.unlinkSync(path.join(q, f)); } catch (e) { /* ignorieren */ } }
  // Zeitstempel im Schluessel: sonst gilt derselbe Zugang auf dem Server als schon angewendet
  return enqueue("assistant", "asst:" + hash(body) + ":" + Date.now(), body, new Date().toISOString());
}
...
module.exports = { enqueueMatch, enqueueDecks, enqueueCollection, enqueueAccount, enqueueAssistant, tokensOf, cardInfoOf, matchCardIds, resendAll, syncAll, flush, connect, disconnect, status, device, baseUrl };

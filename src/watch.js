...
function loadConfig() {
  ...
  let cfgPath = require("./paths").configFile();
  const cli = {};
  let once = false;
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === "--supervised") { /* vom Tray gestartet: darf sich für Updates beenden */ }
...
function syncFlush() {
  ...
  // Auf der Website verbundener Assistent gilt auch hier
  try {
    require("./assistant").pullFromSite().then((v) => { if (v && v.neu) log("Assistent: Zugang für " + v.provider + " von der Website übernommen."); }).catch(() => {});
  } catch (e) { /* ohne Assistent */ }
}
...
function onAccount(a) {
  ...
  if (cfg.syncAccount !== false) {
    try {
      ...
      let icons = null; try { icons = require("./emblems").iconsForSync(a, cfg.outDir); } catch (e) { /* ohne Embleme */ }
      sync.enqueueAccount(icons ? Object.assign({}, a, icons) : a);
    } catch (e) { log("Sync: Konto nicht eingereiht: " + e.message); }
  }
  accountDirty = true;
}
...
async function runSession(pid) {
  ...
  if (!lib.memoryScanSupported()) {
    // macOS/Linux: kein Speicher-Scan – Matches, Decks und Konto kommen aus dem Log, die Sammlung bleibt leer
    log("Sammlung: Speicher-Scan gibt es nur unter Windows – Matches, Decks und Konto werden aus dem Log gelesen.");
    await matchTask;
    tryExportDecks(); buildWeb();
    log("Sitzung beendet.");
    return;
  }
 ...
 * während Arena läuft: ein laufendes Match soll vollständig aufgezeichnet werden.
 */
if (process.argv.includes("--supervised")) {
  const ROOT = path.join(__dirname, "..");
  const stand = () => {
    let m = 0;
    for (const d of ["src", "web"]) {
      let files = [];
      try { files = fs.readdirSync(path.join(ROOT, d)); } catch (e) { continue; }
      for (const f of files) if (/\.(js|css|html)$/.test(f)) { try { m = Math.max(m, fs.statSync(path.join(ROOT, d, f)).mtimeMs); } catch (e) { /* egal */ } }
    }
    return m;
  };
  const beimStart = stand();
  let gemeldet = false;
  setInterval(() => {
    if (stand() <= beimStart) return;
    if (lib.mtgaPid()) { if (!gemeldet) { log("Neuer Programmstand erkannt, Neustart nach dem Spiel."); gemeldet = true; } return; }
    log("Neuer Programmstand erkannt, Watcher startet neu.");

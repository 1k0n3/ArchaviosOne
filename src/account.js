...
class AccountParser {
  ...
  line(l) {
    ...
    else if (pending && pending.name === "RankGetSeasonAndRankDetails" && j.currentSeason) this.season(j.currentSeason, j);
  ...
  season(s, j) {
    // Schritte je Stufe (z. B. Silber 3 = 6 Schritte) für die Pips unter dem Emblem
    // Einträge ohne rankClass sind Bronze (Arena lässt die Standardklasse weg)
    const steps = (list) => { const o = {}; for (const r of Array.isArray(list) ? list : []) if (r && r.level) o[String(r.rankClass || "Bronze").toLowerCase() + ":" + r.level] = r.steps || 0; return o; };
    this.state.season = { ordinal: s.seasonOrdinal || null, start: s.seasonStartTime || null, end: s.seasonEndTime || null, steps: { constructed: steps(j && j.constructedRankInfo), limited: steps(j && j.limitedRankInfo) } };
  }
  periodic(p) {
  ...
  quests(list) {
    this.state.quests = list.map((q) => ({
      ...
      goal: q.goal || 0,
      progress: q.endingProgress != null ? q.endingProgress : (q.startingProgress || 0),
      canSwap: !!q.canSwap,
...
if (require.main === module) {
  ...
  const file = args.find((a) => !a.startsWith("--")) || require("./paths").findLogFile();
  let last = null;
  ...
  (async () => {
    try {
      ...
      if (sync.device()) {
        ...
        let icons = null; try { icons = require("./emblems").iconsForSync(last, outDir); } catch (e) { /* ohne Embleme */ }
        sync.enqueueAccount(icons ? Object.assign({}, last, icons) : last); await sync.flush(); console.log("Website: Kontodaten gesendet" + (icons ? " (mit Bildern aus dem Spiel)" : "") + ".");

...
function findInstallDir() {
  const os = require("os");
  const logDir = path.join(os.homedir(), "AppData", "LocalLow", "Wizards Of The Coast", "MTGA");
  for (const f of ["Player.log", "Player-prev.log"]) {
    ...
    fs.closeSync(fd);
    const m = buf.toString("utf8", 0, n).match(/Mono path\[0\] = '(.+?)[\\/]MTGA_Data[\\/]Managed'/);
...
function indexBundles() {
  ...
  const ab = path.join(dir, "MTGA_Data", "Downloads", "AssetBundle");
...
const UA = "MTGAStats/1.0 (lokales Dashboard; github mtga-stats)";
let lastFetchAt = 0;
...
module.exports = { start, texture, cardImageUrl, prefetch };
